    ...
    <div className="space-y-6" aria-labelledby="attempt-detail-title">
      ...
      {isLoading ? (
        <div className="space-y-3">
          {[1, 2, 3].map((item) => <div key={item} className="h-24 animate-pulse rounded-2xl bg-slate-100 dark:bg-surface/5" />)}
        </div>
      ) : attempt ? (
        <>
          <section className="grid gap-4 md:grid-cols-4">
            {[
              ...
              const valueClassName = 'mt-3 text-2xl font-bold text-content dark:text-white';
              return (
                <article key={item.label} className="rounded-2xl border border-subtle bg-surface p-5 shadow-sm">
                  <Icon className="h-5 w-5 text-accent" />
                  <p className={valueClassName}>{item.value}</p>
                  <p className="text-xs font-semibold text-content-subtle">{item.label}</p>
                </article>
              );
            })}
          </section>
          ...
          <section className="grid gap-5 lg:grid-cols-[1fr_0.55fr]">
            <div className="space-y-4">
              {detail.report?.sourceMaterial ? (
                <section className="rounded-2xl border border-indigo-200 bg-indigo-50/60 p-5 dark:border-indigo-500/25 dark:bg-indigo-500/10">
                  ...
                  <MarkdownContent className="mt-3 max-h-136 overflow-y-auto pr-2 text-sm leading-7 text-content" content={detail.report.sourceMaterial} />
                </section>
              ) : null}
              ...
              {!hasQuestionDetail ? (
                ...
                detail.markedAnswers.map((marked) => {
                  ...
                  return (
                    <article key={marked.questionIndex} className="rounded-2xl border border-subtle bg-surface p-5 shadow-sm">
                      <div className="flex flex-wrap items-center justify-between gap-3">
                        <div>
                          <p className="text-sm font-semibold text-content dark:text-white">Question {marked.questionIndex + 1}</p>
                          <p className="text-xs text-content-subtle">{question.commandWord || 'Exam question'}</p>
                        </div>
                      ...
                      {question.question ? (
                        <MarkdownContent className="mt-4 text-sm font-medium text-content" content={question.question} />
                      ) : null}
                      ...
                      {question.questionType === 'mcq' && question.options?.length ? (
                        <div className="mt-3 grid gap-2">
                          {question.options.map((option, index) => {
                            ...
                            return (
                              <p key={letter} className="rounded-lg border border-subtle px-3 py-2 text-sm">
                      ...
                      <div className="mt-4 rounded-lg bg-surface-sunken p-3 dark:bg-surface/5">
                        <p className="text-xs font-semibold uppercase tracking-wide text-content-subtle">Your answer</p>
                        <p className="mt-1 whitespace-pre-wrap text-sm text-content-muted dark:text-slate-200">{answer || 'No answer entered.'}</p>
                      </div>

                      <div className="mt-4 grid gap-3 md:grid-cols-2">
                        <div className="rounded-lg border border-subtle p-3">
                          <p className="text-xs font-semibold uppercase tracking-wide text-content-subtle">Feedback</p>
                          <p className="mt-1 text-sm text-content-muted dark:text-slate-200">{marked.feedback}</p>
                        </div>
                        <div className="rounded-lg border border-subtle p-3">
                          <p className="text-xs font-semibold uppercase tracking-wide text-content-subtle">Band</p>
                          <p className="mt-1 text-sm font-semibold text-content-muted dark:text-slate-200">{marked.band}</p>
                        </div>
                      </div>

                      {marked.improvements?.length ? (
                        <div className="mt-4">
                          <p className="text-xs font-semibold uppercase tracking-wide text-content-subtle">Improvements</p>
                          <ul className="mt-2 list-disc space-y-1 pl-5 text-sm text-content-muted dark:text-slate-300">
                            {marked.improvements.map((item, index) => <li key={`${item}-${index}`}>{item}</li>)}
                          </ul>
                        </div>
                      ) : null}
            ...
            <aside className="space-y-4">
              <section className="rounded-2xl border border-subtle bg-surface p-5 shadow-sm">
                <h2 className="font-semibold text-content dark:text-white">Weakness Analysis</h2>
                <div className="mt-3 space-y-2">
                  {(detail.report?.weaknessAnalysis?.length ? detail.report.weaknessAnalysis : attempt.weakness_analysis ?? []).length === 0 ? (
                    <p className="text-sm text-content-subtle">No weakness analysis recorded.</p>
                  ) : (
                    (detail.report?.weaknessAnalysis?.length ? detail.report.weaknessAnalysis : attempt.weakness_analysis ?? []).map((item) => (
                      <p key={item} className="rounded-lg border border-slate-100 px-3 py-2 text-sm text-content-muted dark:border-white/6 dark:text-slate-300">
                        {item}
                      </p>
                    ))
                  )}
              ...
              <section className="rounded-2xl border border-subtle bg-surface p-5 shadow-sm">
                <h2 className="font-semibold text-content dark:text-white">Grade Boost Advice</h2>
                <div className="mt-3 space-y-2">
                  {detail.report?.gradeBoostAdvice?.length ? (
                    ...
                    <p className="text-sm text-content-subtle">No grade boost advice recorded for this attempt.</p>
                  )}
                </div>
              </section>
            </aside>
