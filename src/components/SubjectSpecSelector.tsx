...
export function SubjectSpecSelector({
  ...
  return (
    <div>
      {isLoading ? (
        <div className="h-10 animate-pulse rounded-lg bg-slate-100 dark:bg-surface/8" />
      ) : subjects.length === 0 ? (
        ...
        <div className="grid gap-2 sm:grid-cols-[1fr_auto] sm:items-end">
          <SearchSelect
            ...
            inputClassName="mt-1 w-full rounded-lg border border-subtle px-3 py-2 text-sm font-normal outline-none focus:border-accent bg-surface text-content"
          />
          <Link
