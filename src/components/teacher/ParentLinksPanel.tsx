...
export function ParentLinksPanel({ studentId }: { studentId: string }) {
  ...
  if (loading) {
    return <p className="text-xs text-content-subtle">Loading parent links...</p>;
  }

  return (
    <div className="space-y-2 rounded-lg border border-subtle bg-surface-sunken p-3 dark:bg-surface/3">
      <div className="flex items-center justify-between">
        <p className="text-xs font-semibold text-content-muted">Parent links</p>
      ...
      {activeOrPending.length === 0 ? (
        <p className="text-xs text-content-subtle">No parent linked by a teacher for this student yet.</p>
      ) : (
        <div className="space-y-1.5">
          {activeOrPending.map((link) => (
            <div key={link.id} className="flex items-center justify-between rounded-md border border-subtle bg-surface px-2.5 py-1.5 text-xs">
              <div className="flex flex-col">
                {link.status === 'pending' ? (
                  <span className="font-mono font-semibold uppercase tracking-widest text-content">{link.invite_code}</span>
                ) : (
