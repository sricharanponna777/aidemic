...
export function AxisScaleInput({ value, onChange, axisLabel = 'Y-axis' }: AxisScaleInputProps) {
  return (
    <div className="flex flex-wrap items-center gap-3 rounded-lg border border-dashed border-subtle px-3 py-2 text-xs text-content-muted">
      ...
      <label className="flex items-center gap-1">
        ...
        <input
          ...
          className="w-20 rounded border border-subtle bg-transparent px-1.5 py-0.5"
        />
      </label>
      <label className="flex items-center gap-1">
        Interval
      ...
      <span className="text-content-subtle dark:text-content-subtle">Choose a clean interval such as 1, 2, 5, 10, 20 or 50 that fills at least half the grid.</span>
    </div>
  );
}
